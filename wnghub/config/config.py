...
class Config(BaseConfig):
    ...
    class ConfigSchema(Schema):
        ...
        def validate_auth(self, data, **kwargs):
            _verify_auth(data.get('auth_token'), data.get('username'), data.get('password'))
...
def _verify_auth(auth_token, username, password):
    username_pw_provided = _username_pw_provided(
            username, password)
    if username_pw_provided and auth_token is not None:
        raise ValueError(
            'Cannot provide both username and password, '
            'and auth token'
        )
